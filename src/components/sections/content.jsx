...
const ContentSection = React.memo((props) => {
    ...
    const handle_set_selected_file = React.useCallback((active_file) => {
        dispatch(update_active_file(active_file))
        useMainContextIn.handle_set_editor(active_file)
    }, [active_files])

    const handleRemoveFile = React.useCallback((e, file) => {
        e.stopPropagation()
                    ...
                    <PerfectScrollbar className="page-tabs-cont" style={{zIndex: 9}}>
                            {
                                active_files.map(file => 
                                    <div key={file.path} onClick={() => handle_set_selected_file(file)} className={"tab" + (active_file?.path == file.path ? ' active' : '')}>
                                        <span>
                                            {file.icon}
                                        </span>
                                        <span>{file.name}</span>
                                        <span onClick={(e) => handleRemoveFile(e, file)} className={file.is_touched ? 'is_touched': ''}>
                                            <TimesIcon />
